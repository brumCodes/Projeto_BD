...
import {
  ...
  Grid, Card, CardMedia, CardContent, CardActions, Avatar
} from '@mui/material';
...
function DashboardPage({ usuario, onLogout, onVerDetalhes, onVerPerfil }) {
  // AQUI: Adicione estas duas linhas para verificação
  console.log("Objeto usuario recebido:", usuario);
  console.log("URL do avatar:", usuario?.url_avatar);
  ...
  return (
    <ThemeProvider theme={dashboardTheme}>
      ...
      <Box className="dashboard-container">
        <AppBar position="static" sx={{ backgroundColor: '#0d0d18ff', color: 'white', backdropFilter: 'blur(30px)' }}>
        ...
        <IconButton sx={{ padding: 0, mx: 0.5 }} onClick={() => handleToggleLista(filme.id_filme, 'Desejo Ver')}>
            <img 
                ...
                style={{ 
                    width: '28px', 
                    height: '28px', 
                    filter: listStatus[filme.id_filme]?.desejoVer ? 'opacity(0.4)' : 'none' 
                }} 
            />
        </IconButton>
    </Tooltip>
