...
function FilmeDetalhesPage({ filme, onVoltar, usuario }) {
  ...
  //UseEffect pra carregar o status do filme do banco de dados
  useEffect(() => {
    const fetchMovieStatus = async () => {
        
        if (!filme || !filme.id_filme || !usuario || !usuario.id) {
            console.log('Filme ou usuário não definidos, pulando a busca de status.');
            return;
        }
        try {
            console.log(`Buscando status para o filme ID: ${filme.id_filme}...`);
            
            const response = await axios.get(`http://127.0.0.1:5000/api/filmes/${filme.id_filme}/status?usuario_id=${usuario.id}`);
    ...
    fetchMovieStatus();
  }, [filme, usuario]); // usuario como dependencia
  ...
  const toggleListaAPI = async (filmeId, nomeDaLista) => {
      ...
      try {
          const response = await axios.post(`http://127.0.0.1:5000/api/listas/${endpoint}`, {
              ...
              id_usuario: usuario.id //envia o ID do usuário
          });
